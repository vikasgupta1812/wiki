...
monitor the RAM usage, you can run `free -h -s 1`. Every second (`-s 1`), a similar table will be displayed:
...
The ability to monitor the temperature of the GPU has been added to the firmware. [Source](http://raspberrypi.stackexchange.com/a/1847)
...
### Updating all packages wiht pip

[source](http://stackoverflow.com/a/3452888/2356016)

```bash
pip freeze --local | grep -v '^\-e' | cut -d = -f 1  | xargs -n1 pip install -U
```

Newer versions of pip allow you to list outdated packages, though not in "requirements" format
